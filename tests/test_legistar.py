...
ITEMS = {1: [
    {"EventItemTitle": "Rezoning of 500 SW 1st Ave to RMM-25", "EventItemMatterId": 777,
     "EventItemAgendaNote": "250 multifamily units"},
...
def test_map_items():
    out = map_items(SOURCE, EVENTS, ITEMS)
    assert len(out) == 2  # empty-title skipped; filtering happens later in pipeline
    first = out[0]
    assert first.link == "https://fortlauderdale.legistar.com/LegislationDetail.aspx?ID=777"
